    ...
    if($query&&mysql_num_rows($query)==1)
    {
       ...
       if($index!==false)
       {
           //explodes likes and dislikes
            ...
            //if haven't already disliked
            if(array_search($_SESSION['id'], $comment_dislikes[$index])===false)
            {
                //adds like
                if($comment_dislikes[$index][0]=='')
                {
                    $comment_dislikes[$index][0]=$_SESSION['id'];
                    $num_dislikes=1;
                }
                else
                {
                    $comment_dislikes[$index][]=$_SESSION['id'];
