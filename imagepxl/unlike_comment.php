...
if($image_id!='')
{
    ...
    if($query&&mysql_num_rows($query)==1)
    {
        ...
        if($index!==false)
        {
            ...
            $like_index=array_search($_SESSION['id'], $comment_likes[$index]);
            unset($comment_likes[$index][$like_index]);
            ...
            $query=mysql_query("UPDATE images SET comment_likes='$comment_likes' WHERE image_id='$image_id'");
            if(!$query)
            {
                $errors.="Something went wrong. We are working on fixing it";
                send_mail_error("unlike_comment.php: (2): ", mysql_error());
            }
        }
        else
    ...
    {
        $errors.="Something went wrong. We are working on fixing it";
        send_mail_error("unlike_comment.php: (1): ", mysql_error());
    }
    else
